    ...
    if (flag) {
        if (dict[$('.finalexamquestion').text()] !== undefined) {
            $('#rblAnswers label').each((k, v) => {
                if (dict[$('.finalexamquestion').text()].slice(0, 2) === $(v).text().trim().slice(0, 2)) {
                    setTimeout(() => {
                        $(v).trigger('click');
                    }, 200);
                }
            ...
            setTimeout(() => {
                $('#rblAnswers label').each((k, v) => {
                    if ($('#lblCheckAnswerMessage').text() === 'Correct!') {
                        $('.checkmark').each((k, v) => {
                            if ($(v).css('opacity') === '1') {
                                dict[$('.finalexamquestion').text()] = $(v).parent().text().trim();
                            }
                        });
                    } else if (
                        ...
                        $(v).text().trim().slice(0, 2) == $('#lblCheckAnswerMessage').text().slice(-2)
                    ) {
